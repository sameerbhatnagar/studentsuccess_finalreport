...
bad.rows <- which(stringr::str_count(lines,',') == ncol(fmt))
for (i in 1:length(bad.rows)){
  chartorem<-gregexpr(pattern =',',lines[bad.rows[i]])[[1]][5]
  halfstr<-substr(lines[bad.rows[i]],0,chartorem-1)
  # x<-lines[bad.rows[i]] %>% stri_enc_toutf8(is_unknown_8bit=T)
  Encoding(lines[bad.rows[i]])<-'latin1'
  halfstr2<-substr(lines[bad.rows[i]],chartorem+1,str_length(lines[bad.rows[i]]))
  lines[bad.rows[i]]<-stringr::str_c(halfstr,halfstr2,sep="")
}
li<-paste(lines,collapse='\n')
write(li,paste0(path.to.data.directory,clara.table,'_clean','.txt'),ncolumns=8)

etudiant <-fread(paste0(path.to.data.directory,clara.table,'_clean','.txt'),sep = ',',
                 col.names = fmt$V7)
...
# fmt<-fread(paste0(path.to.data.directory,clara.table,'.fmt'))
#
# etudiant_cours_secondaire <-fread(paste0(path.to.data.directory,clara.table,'_clean.txt'),sep = ',',
#                                   col.names = fmt$V7)
rm(bad.rows,chartorem,clara.table,halfstr,halfstr2,i,li,lines,fmt)
...
demo<-etudiant[,.(student_number,Sexe,birth_place,LangueMaternelle,DateNaissance,CodePostal)]
...
save(courses,
     file = paste0('bin/data/course_records_plus_Dawson.RData'))
save(students_last_session,
     file = paste0('bin/data/labelled_students_plus_Dawson.RData'))
rm(list = ls())
